...
def run_migrations():
    ...
    with engine.connect() as conn:
        ...
        # Add emergency_contacts column to medical_profiles table
        try:
            conn.execute(text("ALTER TABLE medical_profiles ADD COLUMN emergency_contacts JSON"))
            conn.commit()
            print("Successfully added emergency_contacts column to medical_profiles.")
        except Exception:
            conn.rollback() 
...
def add_emergency_contact(contact: schemas.EmergencyContact, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if contact.is_primary:
        for c in contacts: c["is_primary"] = False
...
def update_emergency_contact(contact_id: str, contact_data: schemas.EmergencyContact, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for i, c in enumerate(contacts):
        if c["id"] == contact_id:
            ...
            updated["id"] = contact_id # preserve ID
            contacts[i] = updated
            found = True
            break
    ...
    if contact_data.is_primary:
        for i, c in enumerate(contacts):
            if c["id"] != contact_id: contacts[i]["is_primary"] = False
...
def get_appointments(user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return db.query(models.Appointment).filter(models.Appointment.user_id == user_id).order_by(models.Appointment.appointment_date.asc()).all()
...
def update_appointment(appt_id: int, appt_data: AppointmentUpdate, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    db_appt = db.query(models.Appointment).filter(models.Appointment.id == appt_id, models.Appointment.user_id == user_id).first()
    if not db_appt: raise HTTPException(404, "Appointment not found")
...
def update_prescription(p_id: int, p_data: PrescriptionUpdate, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    db_p = db.query(models.Prescription).filter(models.Prescription.id == p_id, models.Prescription.user_id == user_id).first()
    if not db_p: raise HTTPException(404, "Prescription not found")
...
def get_prescription_file(p_id: int, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    # Note: Potentially accessible by verified doctors too, but for now owner-only as per simple spec
    db_p = db.query(models.Prescription).filter(models.Prescription.id == p_id, models.Prescription.user_id == user_id).first()
...
def get_health_records(user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    return db.query(models.HealthRecord).filter(models.HealthRecord.user_id == user_id).order_by(models.HealthRecord.record_date.desc()).all()
...
def get_health_record(record_id: int, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    record = db.query(models.HealthRecord).filter(models.HealthRecord.id == record_id, models.HealthRecord.user_id == user_id).first()
...
def get_health_record_file(record_id: int, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    # Note: In a real emergency view, this might have different auth, but for patient dashboard it's JWT
    record = db.query(models.HealthRecord).filter(models.HealthRecord.id == record_id, models.HealthRecord.user_id == user_id).first()
...
def delete_account(data: UserDelete, user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Cascade is handled by DB in real apps but for safety we can delete manually if not set
    # Here we just delete user, SQLAlchemy should handle relationships if configured properly
    db.delete(user)
    db.commit()
...
def get_access_log(user_id: int = Depends(get_current_user), db: Session = Depends(get_db)):
    return db.query(models.AccessLog).filter(models.AccessLog.user_id == user_id).order_by(models.AccessLog.scanned_at.desc()).limit(10).all()
